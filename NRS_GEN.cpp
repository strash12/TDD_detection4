...
NRS_GEN::NRS_GEN(_parametrs param,int Cellid):
...
void NRS_GEN::NRC( _parametrs param,float** rs_signal_fixed_real, float** rs_signal_fixed_imag, int** rs_space_index)
{
    ...
    int x2[size];
        ...
        for (int k=0; k<M*2+param.Nc;k++){    
            x2[k+31] = ((x2[k+3]+x2[k+2]+x2[k+1]+x2[k])%2);//инициализация последовательности 2

        }
   
    int c[M*2];
        for(int i=0;i<M*2;i++){
            c[i] = (x1[i+param.Nc]+x2[i+param.Nc])%2;//ПИЛОТЫ
        }

        for (int i=0;i<M;i++){
            rs_signal_real[i][o] = (1/sqrt(2))*(1-2*c[2*i+1]);
            rs_signal_imag[i][o] = (1/sqrt(2))*(1-2*c[2*i+1+1]);
        }
